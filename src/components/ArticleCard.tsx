...
export default function ArticleCard(props: any) {
  ...
  return (
    <div className={`relative flex flex-col justify-between rounded-xl text-black dark:text-white ${!props.className && "bg-slate-800"} ${props.className}`}>
      <span className="absolute right-2 top-2 w-8 h-10 flex flex-col justify-center items-center text-slate-200 bg-green-500 rounded text-sm font-light shadow shadow-white"><FaEye className="size-4 text-white" />{props.viewCount}</span>
      ...
      <div className="px-4 py-8 flex flex-col justify-between h-[295px]">
        ...
        <div className="flex justify-between items-center text-sm text-slate-800 dark:text-slate-300 pb-4 border-b border-b-slate-500">
          <Link to={`/team-memmber-profile/${props.author}`}  className="line-clamp-1 flex justify-start items-center gap-1 text-sm hover:text-green-500 dark:hover:text-orange-400"><CiUser className="size-6 text-slate-900 dark:text-slate-50" />{props.author}</Link>
